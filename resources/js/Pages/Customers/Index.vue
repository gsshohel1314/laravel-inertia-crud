...
<template>
    <Head>
        ...
        <meta head-key="description" name="description" content="This is inertia app customer list page" />
    </Head>
                ...
                <table class="w-full text-sm text-left text-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <tr v-for="(customer, index) in customers" :key="customer.id" class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">{{ customer.status ? 'Active' : 'Inactive' }}</td>
                            <td class="px-6 py-4 text-right">
                                <Link :href="route('customers.edit', customer.id)" class="font-medium text-blue-600 hover:underline">Edit</Link>
                                <button @click="deleteCustomer(customer.id)" type="button" class="font-medium text-red-600 hover:underline">Delete</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
